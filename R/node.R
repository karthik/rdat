# Internal functions to look for an install of node, get the most recent version
...
find_node <- function() {
    if (is.null(.node$dir)) {
        ...
        sources <- c(getOption("NodePath"), ifelse(nzchar(sys_node), dirname(sys_node), 
            ""))
        sources_npm <- c(getOption("NpmPath"), ifelse(nzchar(sys_npm), dirname(sys_node), 
            ""))
        if (!is_windows()) {
        ...
        versions <- lapply(sources, function(src) {
            if (file.exists(src)) 
                get_node_version(src) else numeric_version("0")
        })
...
get_node_version <- function(node_dir) {
    ...
    version_info <- system(paste(shQuote(node_path), "--version"), intern = TRUE)
...
#' @import jsonlite
#' @export 
node_fn_load <- function(node_package, node_cmd = node_package, node_dir = "node", 
    r_package = NULL, return_list = TRUE, ...) {
    if (is.null(r_package)) 
    ...
    if (!file.exists(package.json)) {
        nodepackage_path <- list.files(path = nodepath, pattern = node_package, recursive = TRUE, 
            include.dirs = TRUE)
        package.json <- file.path(nodepath, nodepackage_path, "package.json")
        if (!file.exists(package.json)) {
            stop("Node package '", node_package, "' not found in R package '", r_package, 
                "' under directory '", node_dir, "'.")
        }
    }
    ...
    if (is.null(bin)) 
        stop("Command '", bin, "' not found in node package'", node_package, "'.")
    node_command <- do.call(file.path, as.list(c(nodepackage_path, strsplit(bin, 
        "/")[[1]])))
    fn <- function(args = list()) {
        textargs <- ifelse(length(args) > 0, paste0("--", names(args), " ", args, 
            collapse = " "), "")
...
check_node_fn_deps <- function(node_package, node_dir = "node", r_package = NULL, 
    ...
    message(package_name, " requires node ", node_ver_req, ". You have node ", .node$version)
...
system3 <- function(command, args = character(), ...) {
    ...
    return(list(output = output, stdout = readChar(outfile, file.info(outfile)$size), 
